...
const { parseJSON, formatJSON, sortPkgJSON, initializeEslintPlugin } = require('./utils')
...
function getMode (pluginSettings, filenameOrOptions) {
  ...
  if ((pluginSettings[SETTINGS['json-with-comments-files']]).includes(basename)) {
    ...
    return {
      ...
      [@cypress/eslint-plugin-json]: Skipping file due to "json/${SETTINGS['json-with-comments-files']}" setting: 
        ${pluginSettings[SETTINGS['json-with-comments-files']].join(', ')}
      To remove this warning add "**/${basename}" to your \`.eslintignore\` file
    `,
    }
  }

  if ((pluginSettings[SETTINGS['ignore-files']]).includes(basename)) {
    debug('ignored file', basename)

    return {
      message: stripIndent`
      [@cypress/eslint-plugin-json]: Skipping file due to "json/${SETTINGS['ignore-files']}" setting: 
        ${pluginSettings[SETTINGS['ignore-files']].join(', ')}
      To remove this warning add "**/${basename}" to your \`.eslintignore\` file
    `,
    }
  }
