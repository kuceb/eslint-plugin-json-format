...
function execute (file, baseConfig) {
  ...
  const result = cli.executeOnFiles([path.join(__dirname, file)]).results[0]
...
describe('main spec', () => {
  ...
  it('lint big2.bad.json', async () => {
    const filename = './fixtures/big2.bad.json'
    const { result, time } = execute(filename, {
      fix: true,
    })

    expect(result.output).toBe(await getFormatted(filename))
    // expect good perf
    expect(time).toBeLessThan(200, 'good perf')
  })

  it('lint huge.bad.json', async () => {
    const filename = './fixtures/huge.bad.json'
    const { result, time } = execute(filename, {
      fix: true,
    })
    ...
    expect(result.output).toBe(await getFormatted(filename))
    // expect good perf
    expect(time).toBeLessThan(500, 'good perf')
  })
  ...
  describe('configuration', () => {
    ...
    it('@sort order configuration via string', async () => {
      const filename = './fixtures/__package.json'
      const { result } = execute(filename, {
        fix: true,
  ...
  it('lint bad js', () => {
    const { result } = execute('./fixtures/jsdemo.js', {
      rules: {
